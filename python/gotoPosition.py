...
if clientID!=-1:
    ...
    for i in range(4):
        ...
        print("Round ",i)
        print("Before change")
        print("Robot Config: ",robotCurrentConfig)
        print("data to be use: ",data[i])
        inFloats = robotCurrentConfig + data[i]
        print('After change')
        print('Point is', inFloats)
        ...
        if (res==0) and len(path)>0:
            ...
            with open(name,'w',newline='') as csvfile:
                ...
                writer.writerow(['Position X','Position Y','Position Z','Orientation X','Orientaition Y','Orientation Z'])
                for i in range(len(path)-6):
                    #print("Position X", counter, ":", path[i])
                    #print("Position Y", counter, ":", path[i+1])
                    #print("Position Z", counter, ":", path[i+2])
                    #print("Orientation X", counter, ":", path[i+3])
                    #print("Orientation Y", counter, ":", path[i+4])
                    #print("Orientation Z", counter, ":", path[i+5])
                    writer.writerow([path[i],path[i+1],path[i+2],path[i+3],path[i+4],path[i+5]])
                    i+=6
                    counter+=1
                #res,retInts,retFloats,retStrings,retBuffer=vrep.simxCallScriptFunction(clientID,'remoteApiCommandServer',vrep.sim_scripttype_childscript,'visualizePath',[robotHandle,255,0,255],path[i],[],emptyBuff,vrep.simx_opmode_oneshot_wait)
            #lineHandle=retInts[0]
            ...
            runningPath=True
            while runningPath:
                #endEffectorHandle = vrep.simxGetObjectHandle(clientID, 'UR5_link7_visible', vrep.simx_opmode_oneshot_wait)
                #endEffectorPosition = vrep.simxGetObjectPosition(clientID, endEffectorHandle, -1, vrep.simx_opmode_streaming)
